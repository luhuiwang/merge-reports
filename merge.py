...
def get_pdfs(dir):
    ...
    pdfs_list=list(map(lambda path:{"pdf_path":os.path.splitext(path.replace('\\', '/'))[0],
         "pdf_name":os.path.splitext(os.path.split(path)[1])[0]},pdfs))
    part_dict={"part_name":part,"pdfs":pdfs_list}
    return part_dict

if __name__=="__main__":
    parser = argparse.ArgumentParser(description="生成TEX代码")
    ...
    if not os.path.isdir("./build"):
        os.makedirs("./build")
    with open(os.path.join("./build/",args.dirname+".tex"),"w",encoding="utf-8") as fout:
        fout.write(output)
